...
public class OracleMyBatisDAO {
	...
	public List<Dept> selectAll2(SqlSession session) {
		List<Dept> list = session.selectList("com.dept.DeptMapper.selectAll2");
		return list; 
	}

	public List<Dept> multiSelectMap(SqlSession session, HashMap<String, Integer> map) {
		List<Dept> list = session.selectList("com.dept.DeptMapper.multiSelectMap", map);
	...
	public List<Dept> selectTopN(SqlSession session, String loc, int offset, int limit) {
		List<Dept> list = session.selectList("com.dept.DeptMapper.topNLoc", loc, new RowBounds(offset,limit));
		return list;
	}
